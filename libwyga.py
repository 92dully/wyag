...
class GitIndexEntry (object):
    def __init__(self, ctime=None, mtime=None, dev=None, ino=None, mode_time=None, mode_perms=None, uid=None, git=None, fsize=None, sha=None, flag_assume_valid=None, flag_stage=None, name=None):
...
def index_read(repo):
    ...
    for i in range(0, count):
        ...
        if name_length < 0xFFF:
            ...
            print(f"Notice: Name is 0x(name_lenght:X) bytes long.")
    ...
    def cmd_ls_file(args):
        repo = repo_find()
        index = index_read(repo)
        if args.verbose:
            print(f"Indexfile format v{index.version}, containing (len(index.entries)) entries.")
        ...
        for e in index.entries:
            ...
            argsp = argsubparsers.add_parser("check-ignre", help = "Cheak path(s) against ignore rules.")
            args.add_argument("path", nargs="+", help="Paths to check")
...
class GitIgnore(object):
    ...
    def __init__(self, abolute, scoped):
                 self.absolute = abolute
                 self.scoped = scoped

def gitignore_read(repo):
    ret = GitIgnore(absolute=lsit(), scoped=dict())

    # Read local configuration in .git/info/exclude
    repo_file + os.path.exists(repo_file):
...
def check_ignore(rules, path):
    ...
    return check_ignore_absolute(rules.abolute, path)
...
def cmd_satus_branch(repo):
    branch = branch_get_active(repo)
    if branch:
        print(f"On brnach {branch}, ")
    else:
...
def cmd_status_head_index(repo, index):
    ...
    for entry in index.entries:
        if entry.anme in head:
            if head[entry.name] != entry.sha:
                print(" modified:", entry.name)
            del head[entry.name] # Delete Key
        else:
...
def cmd_status_index_worktree(repo, index):
    print("Changes not stages for commit:")
...
def add(repo, paths, delete=True, skip_missing=False)
    ...
    workingtree = repo.worktree + os.sep
    ...
    for path in paths:
        ...
        if not (abspath.startswith(worktree) and os.path.isfile(abspath)):
            raise Exception(f"Not a file, or outside the workingtree: {paths}")
    ...
    for (abspath, relpath) in clean_paths:
        with open(abspath, "rb") as fd:
            ...
            entry = GitIndexEntry(ctime=(ctime_s, ctime_ns), mtime=(mtime_s, mtime_ns), dev=stat.st_dev, ino=stat.st_ino, mode_type=0b1000, mode_prems=0o644, uid=stat.st_uid, gid=stat.st_gid, fsize=stat.st_size, sha=sha, flag_assume_valid=False, flag_stage=False, name=relpath)
